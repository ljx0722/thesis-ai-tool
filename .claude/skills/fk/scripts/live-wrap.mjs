 ...
 *
 * Usage:
 *   npx fk-skills wrap --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: fk wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-fk-variants="' + id + '" data-fk-variant-count="' + count + '" ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' fk-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-fk-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
...
function detectStyleMode(filePath) {
  ...
  if (ext === '.astro') {
    return {
      ...
      styleTag: '<style is:inline data-fk-css="SESSION_ID">',
    };
  }
  return {
    mode: 'scoped',
    styleTag: '<style data-fk-css="SESSION_ID">',
  };
}

function buildCssSelectorPrefixExamples(styleMode, count) {
  if (styleMode !== 'astro-global-prefixed') return [];
  return Array.from({ length: count }, (_, i) => `[data-fk-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-fk-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-fk-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-fk-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-fk-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-fk-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-fk-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-fk-variant="N"] selector prefixes for this styleMode.',
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-fk-variant')) continue;
