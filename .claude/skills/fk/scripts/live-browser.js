/**
 * FK Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__FK_SKILLS_TOKEN__ and window.__FK_SKILLS_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__FK_SKILLS_LIVE_INIT__) return;
  window.__FK_SKILLS_LIVE_INIT__ = true;

  const TOKEN = window.__FK_SKILLS_TOKEN__;
  const PORT = window.__FK_SKILLS_PORT__;
  if (!TOKEN || !PORT) {
    window.__FK_SKILLS_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'fk-live';
  const PICK_CURSOR_CLASS = PREFIX + '-pick-cursor';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__FK_SKILLS_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[fk] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__FK_SKILLS_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__FK_SKILLS_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__FK_SKILLS_VOCAB__) ? window.__FK_SKILLS_VOCAB__ : [];
  const ICONS = {};
  ...
  const domHelpers = window.__FK_SKILLS_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[fk] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__FK_SKILLS_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__FK_SKILLS_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__FK_SKILLS_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-fk-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-fk-placeholder-resize]')?.dataset.fkPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-fk-editable], [data-fk-original-text], [data-fk-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-fk-editable')
        || node.hasAttribute('data-fk-original-text');
      node.removeAttribute('data-fk-editable');
      node.removeAttribute('data-fk-original-text');
      node.removeAttribute('data-fk-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-fk-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.fkPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.fkPlaceholderWidth;
    ...
    placeholder.dataset.fkPlaceholderWidth = 'explicit';
  }
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-fk-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-fk-variant]:not([data-fk-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-fk-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.fkMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.fkPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-fk-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes fk-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: fk-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function buildSavingRow() {
    ...
    const spinner = el('div', {
      ...
      animation: 'fk-spin 0.6s linear infinite',
      flexShrink: '0',
    });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[fk] Refusing to render empty variant cycling state:', reason);
  ...
  //        data-fk-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-fk-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-fk-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-fk-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[fk] Invalid data-fk-params JSON:', err.message);
      return [];
    }
  }
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.fkTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-fk-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.fkEditable = 'true';
      row.el.dataset.fkOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.fkEditable;
      delete row.el.dataset.fkOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.fkOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('fk-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('fk-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('fk-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[fk] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes fk-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[fk] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[fk] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[fk] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[fk] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[fk] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[fk] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[fk] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[fk] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.fkTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.fkEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-fk-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-fk-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-fk-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-fk-variants="' + sessionId + '"]');
    if (!wrapper) return false;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.fkVariant : null;
      if (!v) continue;
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="fk-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('fk-anchor')?.firstElementChild || null;
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-fk-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-fk-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  function loadSvelteRuntime(runtimeModule) {
    const modulePath = runtimeModule || '/src/lib/fk-skills/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-fk-params`
  ...
  async function applySvelteComponentVariantStyle(variantNum) {
    ...
    if (!scopedCss) return;
    const style = document.createElement('style');
    style.dataset.fkSvelteComponentStyle = sessionId;
    style.dataset.fkVariant = String(variantNum);
  ...
  function scopeCssToSveltePreview(css, sessionId) {
    const prefix = '[data-fk-variants="' + String(sessionId).replace(/"/g, '\\"') + '"] ';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[fk] Failed to mount Svelte variant ' + variantNum + ' for ' + sessionId + ':', err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-fk-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[fk] Could not find original element in live DOM.');
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      wrapper.dataset.fkVariants = sessionId;
      wrapper.dataset.fkVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.fkPreview = 'svelte-component';
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.fkComponentMount = sessionId;
      ...
      positionBar();
      saveSession();
      console.log('[fk] Mounted ' + arrivedVariants + ' Svelte component variants.');
    } catch (err) {
      console.error('[fk] Failed to mount Svelte component variants:', err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-fk-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[fk] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- fk-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- fk-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-fk-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[fk] Variant wrapper not found in source file.');
          return;
        }
        ...
        expectedVariants = parseInt(wrapper.dataset.fkVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
        ...
        positionBar();
        saveSession();
        console.log('[fk] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[fk] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__fkFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-fk-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.fkVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-fk-variant="' + index + '"]');
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-fk-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-fk-variants="' + sessionId + '"]') || n.querySelector?.('[data-fk-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-fk-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-fk-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.fkVariants || n.dataset?.fkVariant) {
          ...
          if (n.querySelector?.('[data-fk-variants],[data-fk-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.fkMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.fkVariantCount || '0');
      ...
      if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.fkMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        refreshParamsPanel();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-fk-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Run /fk init to generate one.', 7000);
          console.log('[fk] Live mode connected.');
        ...
        case 'error':
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[fk] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[fk] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[fk] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[fk] Failed to send event:', err);
        throw err;
      }
      console.debug('[fk] Dropped optional live event:', err);
      return null;
    }
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.fkOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__FK_SKILLS_LIVE_ADAPTER__ || '').toLowerCase();
    if (adapter === 'svelte' || adapter === 'sveltekit') return true;
    if (currentPreviewMode === 'svelte-component' || svelteComponentSession) return true;
    const wrapper = el?.closest?.('[data-fk-variants]');
    return wrapper?.dataset?.fkPreview === 'svelte-component';
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[fk] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[fk] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[fk] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[fk] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[fk] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[fk] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-fk-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || acceptWrapper?.dataset?.fkPreview === 'svelte-component';
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-fk-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-fk-variant="' + variantId + '"]');
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-fk-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-fk-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-fk-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-fk-css]');
    if (style && !document.querySelector('style[data-fk-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-fk-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    ...
    if (wrapper && wrapper.dataset.fkPreview !== 'svelte-component') return false;
  ...
  function cleanup() {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-fk-variants="' + cleanupSessionId + '"]');
      if (wrapper) wrapper.style.display = 'none';
      setTimeout(function() {
        if (!cleanupSessionId) return;
        const lateWrapper = document.querySelector('[data-fk-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-fk-variant="original"]');
        if (orig) {
  ...
  // If a [data-fk-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.fkVariants;
    ...
    // wrapper holds a single mount target, not [data-fk-variant] nodes,
    ...
    if (wrapper.dataset.fkPreview === 'svelte-component'
    ...
    if (wrapper.dataset.fkPreview === 'svelte-component') {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.fkVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.fkVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-fk-variant]:not([data-fk-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[fk] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'fk-live-pick';
  const INTERACTION_PREFS_KEY = 'fk-live-interaction';
  ...
  function attachSteerFocusGuard() {
    if (window.__FK_SKILLS_STEER_FOCUS_GUARD__) return;
    window.__FK_SKILLS_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('fk-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[fk.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__FK_SKILLS_STEER_FOCUS_DEBUG__) return;
    window.__FK_SKILLS_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'fk-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[fk.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[fk.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes fk-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes fk-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes fk-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: fk-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: fk-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', connected
      ? 'FK live mode'
      : 'FK live mode - agent not polling');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes fk-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: fk-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    brand.setAttribute('aria-label', 'FK live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.fk-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'fk-skills-ready'
  ...
  function teardown() {
    ...
    window.__FK_SKILLS_LIVE_INIT__ = false;
    console.log('[fk] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .fk-skills/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'fk-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .fk-skills/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/fk document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .fk-skills/design.json.</strong> Run <code>/fk document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/fk document</code> also generates a <code>.fk-skills/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .fk-skills/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[fk] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[fk] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[fk] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
