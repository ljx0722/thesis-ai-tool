    ...
    if (p === '/manual-edit-commit' && req.method === 'POST') {
      ...
      (async () => {
        ...
        try {
          ...
          const requestedMode = (envValue.FK_SKILLS_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
          const useChatRoute = requestedMode === 'chat'
            || (requestedMode === 'auto' && chatAgentLikelyActive());
          if (useChatRoute) {
            routedProvider = 'chat';
            const timeoutMs = Number(envValue.FK_SKILLS_LIVE_COPY_AGENT_TIMEOUT_MS || 120000);
