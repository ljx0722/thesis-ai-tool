...
export function buildCopyEditBatchPrompt(batch, { cwd = process.cwd() } = {}) {
  ...
  return [
    ...
    '- Never copy browser edit-mode scaffolding into source: no contenteditable, data-fk-* markers, wrapper variants, generated style/script tags, or runtime-only attributes.',
    ...
    '- If package.json defines scripts.fk:manual-edit-validate, it must pass after edits.',
    '- Check for leftover fk-carbonize markers or variant wrapper markers in touched files.',
    '',
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  if (provider === 'mock') {
    const delayMs = Number(env.FK_SKILLS_LIVE_COPY_AGENT_MOCK_DELAY_MS || 0);
  ...
  const outDir = opts.outDir || fs.mkdtempSync(path.join(os.tmpdir(), 'fk-copy-batch-'));
...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    const markerMatch = findLeftoverFkMarker(content);
    if (markerMatch) failures.push({ file: relativeFile, reason: 'leftover_fk_marker', marker: markerMatch });
...
function findLeftoverFkMarker(content) {
  const commentMarker = content.match(/^\s*(?:<!--|\{\/\*)\s*(?:fk|impeccable)-carbonize-(?:start|end)\b|^\s*(?:<!--|\{\/\*)\s*(?:fk|impeccable)-variants-(?:start|end)\b/m);
  if (commentMarker) return commentMarker[0];

  const attrPattern = /\bdata-fk-(?:variants?|original-text|editable|text-wrap)\s*=/g;
...
function readManualEditValidationScript(cwd) {
  ...
  try {
    ...
    const script = pkg?.scripts?.['fk:manual-edit-validate']
...
function stripLiveRuntimeHtml(html) {
  ...
  return html
    .replace(/\sdata-fk-(?:original-text|editable|text-wrap)(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?/g, '')
...
function mockBatchResult(batch, env, cwd = process.cwd()) {
  ...
  const raw = env.FK_SKILLS_LIVE_COPY_AGENT_MOCK_RESULT;
  if (raw) {
    const parsed = parseCopyEditBatchResult(raw);
    if (parsed) return parsed;
    throw new Error('Invalid FK_SKILLS_LIVE_COPY_AGENT_MOCK_RESULT JSON');
  }
  return {
    status: 'done',
...
function applyMockWrites(env, cwd) {
  const raw = env.FK_SKILLS_LIVE_COPY_AGENT_MOCK_WRITES;
  if (!raw) return;
  const writes = tryParseJson(raw);
  if (!writes || typeof writes !== 'object' || Array.isArray(writes)) {
    throw new Error('Invalid FK_SKILLS_LIVE_COPY_AGENT_MOCK_WRITES JSON');
  }
...
export function chooseCopyEditAgent({
  ...
  const mode = (env.FK_SKILLS_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
...
function runCodex(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = [
    ...
    '-c', `model_reasoning_effort="${env.FK_SKILLS_LIVE_COPY_AGENT_EFFORT || 'low'}"`,
  ];
  if (env.FK_SKILLS_LIVE_COPY_AGENT_MODEL) {
    args.push('--model', env.FK_SKILLS_LIVE_COPY_AGENT_MODEL);
  }
  args.push('-');
...
export function describeNoProviderError({
  ...
  if (chatAvailable()) {
    lines.push('  • Chat: an FK live session is polling but selection chose another provider — unexpected; please report.');
  } else {
    lines.push('  • Chat: no FK live session is currently polling on this server. Start FK live in your chat to route Apply through the chat agent.');
  }
  lines.push('Fix one of the above, or set FK_SKILLS_LIVE_COPY_AGENT=mock for tests.');
  return lines.join('\n');
}
