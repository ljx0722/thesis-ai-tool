"use strict";
(function(){
  var state={view:'local',anchor:null,claim:null,results:[],loading:false,error:'',cartOpen:false};
  ...
  function activateSelection(anchor){state.anchor=anchor;state.claim=claimFromAnchor(anchor);state.results=[];state.error='';state.view='local';open({mode:'local',keepSelection:true});}
  ...
  function installSelectionToolbar(){
    ...
    bar.addEventListener('click',function(e){var btn=e.target.closest('button');if(!btn)return;var anchor=bar._anchor;if(!anchor)return;bar.hidden=true;
      ...
      activateSelection(anchor);if(btn.dataset.action==='evidence')state.view='matrix';render();
    });
  }
  ...
  function audit(){
    var scope=window.getManuscriptScope?window.getManuscriptScope():{paragraphs:[]},items={};scope.paragraphs.forEach(function(p){p.sentences.forEach(function(s){var text=s.text||'',hasRef=/\[\d+(?:[,，、-]\d+)*\]/.test(text),needs=!hasRef&&text.length>=18&&(/研究|数据|表明|发现|认为|指出|影响|导致|方法|模型|理论|比例|增长|下降/.test(text));if(!needs)return;var id='audit-'+simpleTextHash((scope.revisionId||'')+':'+p.paragraphId+':'+s.sentenceId+':'+text);items[id]={id:id,paragraphId:p.paragraphId,sentenceId:s.sentenceId,text:text,status:'pending',reason:'包含可验证的事实、关系或研究判断，但未检测到引用',createdAt:new Date().toISOString()};});});
    persist(function(lit){Object.keys(items).forEach(function(id){if(!lit.audits[id])lit.audits[id]=items[id];});return lit;}).then(function(){state.view='audit';render();}).catch(reportSaveError);
  }
  function auditToClaim(id){var lit=artifact(),item=lit.audits[id],entry=(window._treeIndex.paragraphs||[]).find(function(e){return(e.node||e).paragraphId===item.paragraphId;});if(!item||!entry)return;var p=entry.node||entry,s=(p.sentences||[]).find(function(x){return x.sentenceId===item.sentenceId;});if(!s)return;var text=citationPlainText(p.el),start=text.indexOf(s.text),anchor={revisionId:s.revisionId,paragraphId:p.paragraphId,structuralPath:p.structuralPath,startOffset:start,endOffset:start+s.text.length,quote:s.text,prefix:text.substring(Math.max(0,start-32),start),suffix:text.substring(start+s.text.length,start+s.text.length+32),normalizedTextHash:simpleTextHash(text)};activateSelection(anchor);}
  ...
  function tabs(){return'<div class="literature-tabs" role="tablist">'+[['local','局部反查'],['audit','全文审计'],['matrix','证据矩阵'],['format','格式与导出']].map(function(item){return'<button role="tab" aria-selected="'+(state.view===item[0])+'" class="'+(state.view===item[0]?'active':'')+'" data-view="'+item[0]+'">'+item[1]+'</button>';}).join('')+'</div>';}
  ...
  function auditView(){var lit=artifact(),items=Object.keys(lit.audits).map(function(k){return lit.audits[k];}).filter(function(x){return x.status==='pending';});return'<div class="literature-section-head"><div><strong>全文引用审计</strong><p>只识别可能缺引的位置，不自动检索或插入。</p></div><button type="button" data-action="run-audit">运行审计</button></div>'+(items.length?'<div class="audit-list">'+items.map(function(item){return'<article class="audit-item"><p>'+esc(item.text)+'</p><span>'+esc(item.reason)+'</span><div><button data-audit-search="'+item.id+'">为此句找文献</button><button data-audit-dismiss="'+item.id+'" data-status="not-needed">无需引用</button><button data-audit-dismiss="'+item.id+'" data-status="later">稍后处理</button></div></article>';}).join('')+'</div>':'<div class="literature-empty"><strong>尚无待处理项</strong><p>点击“运行审计”，系统会按句子检查事实、因果、方法和研究判断。</p></div>');}
  ...
  function cartBar(){var lit=artifact(),count=lit.cart.paperIds.length;return'<div class="literature-cart"><button type="button" data-action="cart">候选篮 <strong>'+count+'</strong></button><span>'+Object.keys(lit.evidenceLinks).length+' 条已接受证据</span></div>';}
  function render(){var root=panel();if(!root)return;root.innerHTML=tabs()+'<div class="literature-body">'+(state.view==='local'?localView():state.view==='audit'?auditView():state.view==='matrix'?matrixView():formatView())+'</div>'+cartBar();}
  function open(options){options=options||{};state.view=options.mode||state.view;if(!options.keepSelection){var anchor=selectedAnchor();if(anchor){state.anchor=anchor;state.claim=claimFromAnchor(anchor);}}
    if(typeof switchModule==='function')switchModule('references');setHeader();var root=panel();if(root)root.style.display='flex';var oldRefs=refs();if(oldRefs)oldRefs.style.display='none';render();
  }
  function bind(){var root=panel();if(!root)return;root.addEventListener('click',function(e){var tab=e.target.closest('[data-view]');if(tab){state.view=tab.dataset.view;render();return;}var action=e.target.closest('[data-action]');if(action){if(action.dataset.action==='search')searchClaim();else if(action.dataset.action==='audit'||action.dataset.action==='run-audit')audit();else if(action.dataset.action==='export')exportBibliography();return;}var cart=e.target.closest('[data-cart]');if(cart){toggleCart(cart.dataset.cart);return;}var confirm=e.target.closest('[data-confirm]');if(confirm){confirmCitation(confirm.dataset.confirm);return;}var auditSearch=e.target.closest('[data-audit-search]');if(auditSearch){auditToClaim(auditSearch.dataset.auditSearch);return;}var dismiss=e.target.closest('[data-audit-dismiss]');if(dismiss){dismissAudit(dismiss.dataset.auditDismiss,dismiss.dataset.status);}});root.addEventListener('change',function(e){if(e.target.matches('[data-relation]'))setRelation(e.target.dataset.relation,e.target.value);if(e.target.id==='lwStyle'){persist(function(lit){lit.settings.citationStyle=e.target.value;return lit;}).then(render).catch(reportSaveError);}});}
  ...
  window.LiteratureWorkbench={open:open,render:render,audit:audit,buildQueryPlan:buildQueryPlan,normalizeDoi:normalizeDoi,paperId:paperId,formatPaper:formatPaper};
