...
(function(){
  ...
  function audit(){
    try{
      var scope=window.getManuscriptScope?window.getManuscriptScope():{paragraphs:[]},items={};
      if(!scope.paragraphs||!scope.paragraphs.length){state.view='audit';state.error='请先导入论文正文后再运行审计';render();if(typeof ttp==='function')ttp(state.error);return;}
      scope.paragraphs.forEach(function(p){(p.sentences||[]).forEach(function(s){var text=s.text||'',hasRef=/\[\d+(?:[,，、-]\d+)*\]/.test(text),needs=!hasRef&&text.length>=18&&(/研究|数据|表明|发现|认为|指出|影响|导致|方法|模型|理论|比例|增长|下降/.test(text));if(!needs)return;var hash=typeof simpleTextHash==='function'?simpleTextHash((scope.revisionId||'')+':'+p.paragraphId+':'+s.sentenceId+':'+text):String(p.paragraphId)+'-'+String(s.sentenceId);var id='audit-'+hash;items[id]={id:id,paragraphId:p.paragraphId,sentenceId:s.sentenceId,text:text,status:'pending',reason:'包含可验证的事实、关系或研究判断，但未检测到引用',createdAt:new Date().toISOString()};});});
      state.view='audit';state.error='';
      if(!currentProject()){render();if(typeof ttp==='function')ttp('审计完成：发现 '+Object.keys(items).length+' 个可能缺引位置（未登录项目，结果仅本次有效）');return;}
      persist(function(lit){Object.keys(items).forEach(function(id){if(!lit.audits[id])lit.audits[id]=items[id];});return lit;}).then(function(){render();if(typeof ttp==='function')ttp('审计完成：发现 '+Object.keys(items).length+' 个可能缺引位置');}).catch(reportSaveError);
