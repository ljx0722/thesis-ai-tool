...
(function(){
  ...
  function artifact(){return window.ThesisProject&&ThesisProject.getLiteratureArtifact?ThesisProject.getLiteratureArtifact():{claims:{},papers:{},evidenceLinks:{},occurrences:{},audits:{},searchRuns:{},annotations:{},cart:{paperIds:[],selections:{}},settings:{citationStyle:'gbt7714-numeric'},bibliography:{includedPaperIds:[]}};}
  ...
  function activateSelection(anchor,autoSearch){state.anchor=anchor;state.claim=claimFromAnchor(anchor);state.results=[];state.error='';state.searchStatus='idle';state.view='local';open({mode:'local',keepSelection:true});if(autoSearch)setTimeout(function(){searchClaim();},0);}
  ...
  function emptyLocal(){return'<div class="literature-empty"><strong>从研究问题或一句原文开始找依据</strong><p>点击顶部“检索”可根据当前项目想法生成论点；也可以在左侧正文中选择句子或段落后点击“为此处找文献”。系统不会自动修改正文。</p><button type="button" data-action="manual-claim">手动填写检索论点</button><button type="button" data-action="audit">先做全文引用审计</button></div>';}
  ...
  function open(options){options=options||{};state.view=options.mode||state.view||'local';if(!options.keepSelection){var anchor=selectedAnchor();state.anchor=anchor||null;state.claim=anchor?claimFromAnchor(anchor):(options.seedProject?projectClaim():state.claim);state.results=[];state.error='';state.searchStatus='idle';}
