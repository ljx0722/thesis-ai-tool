"use strict";
(function(){
  var state={view:'local',anchor:null,claim:null,results:[],loading:false,error:'',searchStatus:'idle',cartOpen:false,annotationDraft:null,annotationFilter:'all',annotationStatuses:{},auditRun:{status:'idle',processed:0,total:0,findings:0,startedAt:null,completedAt:null,summary:null}};
  ...
  function artifact(){return window.ThesisProject&&ThesisProject.getLiteratureArtifact?ThesisProject.getLiteratureArtifact():{claims:{},papers:{},evidenceLinks:{},occurrences:{},audits:{},searchRuns:{},searchSessions:{},chapterAssignments:{},annotations:{},cart:{paperIds:[],selections:{}},settings:{citationStyle:'gbt7714-numeric'},bibliography:{includedPaperIds:[]}};}
  ...
  function resetRuntime(){
    clearAnnotationHighlights();state.anchor=null;state.claim=null;state.results=[];state.loading=false;state.error='';state.searchStatus='idle';state.annotationDraft=null;state.annotationStatuses={};var bar=document.getElementById('literatureSelectionBar');if(bar){bar.hidden=true;bar._anchor=null;}if(panel()&&panel().style.display!=='none')render();
  }
  ...
  function tabs(){return'<div class="literature-tabs" role="tablist">'+[['local','局部反查'],['annotations','阅读批注'],['imported','已导入'],['audit','全文审计'],['matrix','证据矩阵'],['format','格式与导出']].map(function(item){return'<button role="tab" aria-selected="'+(state.view===item[0])+'" class="'+(state.view===item[0]?'active':'')+'" data-view="'+item[0]+'">'+item[1]+'</button>';}).join('')+'</div>';}
  ...
  function render(){var root=panel();if(!root)return;var body=state.view==='local'?localView():state.view==='annotations'?annotationsView():state.view==='imported'?importedView():state.view==='audit'?auditView():state.view==='matrix'?matrixView():formatView();root.innerHTML=tabs()+'<div class="literature-body">'+body+'</div>'+cartBar();}
  function open(options){options=options||{};state.view=options.mode||state.view||'local';if(!options.keepSelection){var anchor=options.anchor||selectedAnchor();state.anchor=anchor||null;state.claim=options.claim||((anchor?claimFromAnchor(anchor):(options.seedProject?projectClaim():state.claim)));state.results=[];state.error='';state.searchStatus='idle';}
