...
function runParagraphAnalysis(container) {
  ...
  var stats = { totalParas: 0, longParas: 0, shortParas: 0, debrisParas: 0, totalSents: 0, longSents: 0, avgSentLen: 0 };
  var paraData = [];
  ...
  if (box) {
    ...
    for (var pi = 0; pi < paras.length; pi++) {
      ...
      if (/^(\d+(?:\.\d+)*\s*|第[一二三四五六七八九十\d]+章\s*)$/.test(pt) ||
          (pt.length<25&&/^[^。！？\.\?\!]{1,25}$/.test(pt)&&!/[a-z一-鿿]{3}/.test(pt))) stats.debrisParas++;
    }
    stats.totalSents = ts;
  ...
  // mammoth 标题拆分残留诊断
  if(stats.debrisParas>3){
    h+='<h4>⚠ 标题拆分残留</h4>';
    h+='<div class="finding warn">⚠ 检测到 '+stats.debrisParas+' 个可能的标题拆分残留段落（纯编号或孤立短文本）。原因：Word 未套用“标题1/2/3”样式，mammoth 将编号与标题文字拆为独立段落。建议回 Word 统一设置标题样式后重新上传。</div>';
  }
