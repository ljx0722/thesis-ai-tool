...
function openThemeStudio(){var p=loadPreferences();_buddyThemePreviewBaseline=Object.assign({},p);_buddyThemeSaving=false;fillPreferenceForm(p);var panel=document.getElementById('themeStudio'),backdrop=document.getElementById('themeStudioBackdrop');if(!panel||!backdrop){if(typeof ttp==='function')ttp('外观设置加载失败，请刷新后重试');return;}panel.classList.add('open');panel.setAttribute('aria-hidden','false');backdrop.classList.add('open');requestAnimationFrame(function(){var first=panel.querySelector('select,button,input');if(first)first.focus();});}
function closeThemeStudio(){if(!_buddyThemeSaving&&_buddyThemePreviewBaseline)applyPreferences(_buddyThemePreviewBaseline);var panel=document.getElementById('themeStudio'),backdrop=document.getElementById('themeStudioBackdrop');if(panel){panel.classList.remove('open');panel.setAttribute('aria-hidden','true');}if(backdrop)backdrop.classList.remove('open');_buddyThemePreviewBaseline=null;_buddyThemeSaving=false;}
